...
describe("Battle Command v4 accessibility and interaction", () => {
  ...
  it("submits selected weight-six fleets and builds an ordered two-step Condense plan", async () => {
    const fetchMock = vi.fn().mockResolvedValue(response(battleViewFixture("kinetic"), 201));
    ...
    const scoutCard = screen.getByText(/Condense 2\/2/).closest("article")!;
    expect(within(scoutCard).getByRole("button", { name: "Move on grid" })).toBeDisabled();
  });
