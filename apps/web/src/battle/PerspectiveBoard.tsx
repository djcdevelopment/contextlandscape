...
import type { AttentionV4Coordinate, AttentionV4KineticAction, BattleCommandV3View } from "@landscape/contracts";
...
export function PerspectiveBoard({
  ...
  useEffect(() => {
    ...
    for (const destination of plannedDestinationOrders.values()) {
      const { x, y } = destination.coordinate;
      const corners = [point(x, y), point(x + 1, y), point(x + 1, y + 1), point(x, y + 1)];
      context.save();
      context.beginPath(); corners.forEach((item, index) => index ? context.lineTo(item.x, item.y) : context.moveTo(item.x, item.y)); context.closePath();
      context.fillStyle = "rgba(239,201,108,.08)"; context.fill();
      context.strokeStyle = "#efc96c"; context.lineWidth = Math.max(1.5, 2 * uiScale); context.setLineDash([6 * uiScale, 4 * uiScale]); context.stroke();
      context.restore();
    }
  ...
  const activate = (coordinate: AttentionV4Coordinate) => {
    ...
    if (!readOnly && (view.projection.phase === "artillery" || (!unit && !artifact))) onCell(coordinate);
  };
  ...
  return <section className="perspective-board" aria-label="Player-edge perspective battlefield" data-read-only={readOnly || undefined}>
    ...
    <div className="perspective-canvas-shell">
      ...
      <span id="perspective-grid-help" className="sr-only">Use arrow keys and Enter to select and inspect. Drag to pan and use Home to reset.</span><span className="sr-only" aria-live="polite">Coordinate {focus.x}, {focus.y}</span>
      <div className="perspective-semantic-grid sr-only" role="grid" aria-label="10 by 10 operational field" aria-describedby="perspective-grid-help" aria-readonly={readOnly}>{Array.from({ length: 10 }, (_, y) => <div role="row" key={y}>{Array.from({ length: 10 }, (_, x) => {
    ...
    <p className="perspective-help">{readOnly ? "Read-only · select units and artifacts to inspect · " : ""}Drag to pan · wheel to zoom · limited yaw preserves orientation · Home resets to your edge</p>
  </section>;
}
