[CmdletBinding()]
param(
  [ValidateSet('all', 'stationary-train', 'capacity-train', 'holdout', 'v3-shape', 'v3-artillery-causal', 'v3-artillery-mechanism-screen', 'v3-desperation-artillery')]
...
function New-DefaultExperiments {
  $definitions = @(
    ...
    @{ Kind = 'v3-desperation-artillery'; RunsPerSeed = 144; DefaultSeeds = 5000; DefaultSeedStart = 50000000 }
  )
...
try {
  ...
  foreach ($experiment in @($experiments | Where-Object Kind -in @('v3-artillery-causal', 'v3-artillery-mechanism-screen', 'v3-desperation-artillery'))) {
    ...
    $preflightSeeds = if ($experiment.Kind -eq 'v3-desperation-artillery') { 64 } else { 1 }
    $preparePreflight = @('-p', $project, '-f', 'infra/compose.lab.yml', 'run', '--rm') + $provenanceEnv + @(
      'worker', 'node', 'apps/lab/dist/main.js', "--attention-campaign=$($experiment.Kind)",
      "--matrix=$preflightId", "--runs=$preflightSeeds", '--shards=1', "--seed-start=$($experiment.SeedStart)",
      "--canonical=$canonicalValue", '--prepare=true'
    )
    ...
    [double]$bytesPerRun = (Get-Item -LiteralPath $preflightShard).Length / [double]($experiment.RunsPerSeed * $preflightSeeds)
  ...
  foreach ($experiment in $experiments) {
    ...
    if ($experiment.Kind -in @('v3-artillery-causal', 'v3-artillery-mechanism-screen', 'v3-desperation-artillery')) {
