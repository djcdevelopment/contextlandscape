...
const OUTPUT_DIRECTORY = resolve("data/experiments/attention-v3-stage-c5-three-response-holdout");
const WORLD_STREAM_NAMESPACE = "attention-v3-stage-c5-three-response-world-v1";
const PARENT_STAGE_C4_REPORT_HASH = "sha256:9be23296c0eafd9def6253e2f73fecd0c86afba5b4b998151727816f468d9cd6";
...
const holdoutPressureSamples = [
  { id: "binary-sound-35", objectiveCoupling: "binary-front", soundnessRate: 0.35 },
  { id: "binary-sound-70", objectiveCoupling: "binary-front", soundnessRate: 0.70 },
  { id: "binary-sound-95", objectiveCoupling: "binary-front", soundnessRate: 0.95 },
  { id: "global-sound-25", objectiveCoupling: "global", soundnessRate: 0.25 },
  { id: "global-sound-65", objectiveCoupling: "global", soundnessRate: 0.65 },
  { id: "global-sound-85", objectiveCoupling: "global", soundnessRate: 0.85 },
  { id: "distance-sound-35", objectiveCoupling: "distance-weighted-front", soundnessRate: 0.35 },
  { id: "distance-sound-75", objectiveCoupling: "distance-weighted-front", soundnessRate: 0.75 },
  { id: "distance-sound-95", objectiveCoupling: "distance-weighted-front", soundnessRate: 0.95 }
];
const aimModes = ["cluster-center", "artifact-density", "far-objective"];
const holdoutCommandModes = ["confidence-threshold", "local-verify"];
const trainSeeds = Array.from({ length: 16 }, (_, index) => 404_000 + index);
const holdoutSeeds = Array.from({ length: 16 }, (_, index) => 404_016 + index);
...
const planDefinition = {
  schemaVersion: 1,
  experiment: "attention-v3-stage-c5-three-response-disjoint-soundness-holdout",
  modelVersion: "duel-capacity-v3-experimental",
  resolverVersion: ATTENTION_V3_ARTILLERY_RESOLVER_VERSION,
  parentStageC4ReportHash: PARENT_STAGE_C4_REPORT_HASH,
  staticPolicies,
  ruleCandidates,
...
const planId = `attention-v3-stage-c5-${planHash.slice(7, 23)}`;
...
const reportWithoutHash = {
  schemaVersion: 1,
  artifactKind: "attention-v3-stage-c5-three-response-holdout-report",
...
const assessment = `# Attention v3 Stage-C5 three-response disjoint-soundness holdout\n\n` +
  ...
  `Parent Stage-C4 report: \`${PARENT_STAGE_C4_REPORT_HASH}\`\n\n` +
  `Training executed ${observedTrainWorlds.toLocaleString("en-US")} worlds / ${observedTrainRuns.toLocaleString("en-US")} matches and ${plannedCandidateEvaluations.toLocaleString("en-US")} exact candidate-world lookups. The combined policy was then instantiated once for ${observedHoldoutWorlds.toLocaleString("en-US")} untouched worlds / ${observedHoldoutRuns.toLocaleString("en-US")} actual matches on a disjoint soundness grid.\n\n` +
  ...
  `- The worst coupling x aim x doctrine gap was ${Math.min(...couplingAimDoctrine.map((row) => row.adaptiveGap)).toFixed(4)}; the worst disjoint pressure x doctrine gap was ${Math.min(...pressureDoctrine.map((row) => row.adaptiveGap)).toFixed(4)}.\n` +
...
function thresholdsSvg(rows) {
  ...
  <text x="38" y="47" fill="#ffffff" font-size="28" font-weight="700" font-family="Segoe UI, sans-serif">Stage C5 · selected public three-response thresholds</text>
...
function atlasSvg(rows) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  <rect width="100%" height="100%" fill="#09111f"/>
  <text x="32" y="47" fill="#ffffff" font-size="28" font-weight="700" font-family="Segoe UI, sans-serif">Stage C5 - disjoint-soundness holdout atlas</text>
  <text x="32" y="77" fill="#94a3c7" font-size="16" font-family="Segoe UI, sans-serif">Orange: best static | cyan: within 0.075 | red: frozen holdout failure</text>
  ${bars}
</svg>\n`;
}
...
await writeFile(resolve(OUTPUT_DIRECTORY, "holdout-atlas.svg"), atlasSvg(pressureDoctrine), "utf8");
