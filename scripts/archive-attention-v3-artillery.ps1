...
if ($report.campaignKind -ne 'v3-artillery-causal' -or $report.runs -ne 9216000 -or @($report.shards).Count -ne 12) { throw 'Matrix is not the complete artillery campaign' }
if ($assessment.source.reportHash -ne $report.reportHash -or $assessment.source.manifestHash -ne $report.manifestHash) { throw 'Assessment does not bind the completed matrix' }
...
$manifest = [ordered]@{
  schemaVersion = 1
  archiveKind = 'attention-v3-artillery-causal-five-drift'
...
$manifestFileHash = Get-Hash $manifestPath
...
try {
  ...
  foreach ($entry in $entries) {
    ...
    if (!(Test-Path -LiteralPath $restored -PathType Leaf) -or (Get-Hash $restored) -ne $entry.sha256) { throw "Restored hash mismatch: $($entry.path)" }
  }
  $restoredManifest = Join-Path $verifyPath (Get-RelativePath $manifestPath)
  if ((Get-Hash $restoredManifest) -ne $manifestFileHash) { throw 'Restored archive manifest mismatch' }
} finally {
...
$sidecar = [ordered]@{
  ...
  archiveManifestHash = $manifestFileHash
